...
database1 = 'wind_data'
connect_info1 = 'mysql+pymysql://{}:{}@{}:{}/{}?charset=utf8'.format(username, pwd, host, '3306', database1)
wind_data = create_engine(connect_info1)

database2 = 'market_base'
connect_info2 = 'mysql+pymysql://{}:{}@{}:{}/{}?charset=utf8'.format(username, pwd, host, '3306', database2)
market_base = create_engine(connect_info2)

database3 = 'research'
connect_info3 = 'mysql+pymysql://{}:{}@{}:{}/{}?charset=utf8'.format(username, pwd, host, '3306', database3)
research = create_engine(connect_info3)

database4 = 'std_market_data'
connect_info4 = 'mysql+pymysql://{}:{}@{}:{}/{}?charset=utf8'.format(username, pwd, host, '3306', database4)
...
pwd5 = "Efpm2021$"
database5 = 'efpm'
connect_info = 'mysql+pymysql://{}:{}@{}:{}/{}?charset=utf8'.format(username5, pwd5, host5, '3306', database5)
...
def Option_Info_With_Same_Underlying(underlying, tradingdate):
    ...
    ''' % (underlying, tradingdate, tradingdate)
    ...
    info.loc[:, 'options_type'] = info.options_type.apply(lambda x: 'c' if x == '1' else 'p')
    ...
    info.loc[:, 'trading_ttm'] = info.apply(lambda x: np.where(tradingDay == x['expiredate'])[0][0] - np.where(tradingDay == x['trading_date'])[0][0], axis=1)

    info = info.rename(
        columns={'instrument_id': 'wind_code', 'instrument_code': 'instrument_id', 'options_type': 'option_type',
                 'opendate': 'listed_date', 'expiredate': 'expire_date'})
...
def instrument_info(inst_id: str):
    ...
    """
    SQL_cmd = '''SELECT COALESCE(instrument_code, instrument_id) instrument_code, 
    options_type, strike_price, opendate, expiredate, 
...
def calculate_trading_days(begin_date: str, end_date: str):
    ...
    :return:
    """
    trading_days = np.where(tradingDay == end_date)[0][0] - np.where(tradingDay == begin_date)[0][0]
    return trading_days
...
def Spot_Raw_Data(underlying: str, tradingdate: str):
    ...
    FutureData = pd.read_csv(future_path, usecols=['id', 'seq_no', 'bid_price1', 'ask_price1'],
    ...
    FutureData.loc[:, 'FutureMidPrice'] = FutureData.apply(lambda row: max(row["bid_price1"], row["ask_price1"]) if row["bid_price1"] * row["ask_price1"] == 0 else 0.5*(row["bid_price1"]+row["ask_price1"]), axis=1)
...
def Risk_Free_Rate(underlying):
    ...
    r = 0.025*0.5*(margin_ratio.loc[0, 'long_margin_ratio'] + margin_ratio.loc[0, 'short_margin_ratio'])
...
def IV(price, F, K, t, r, flag):
    try:
        ...
        iv = BSMiv.implied_volatility(price, F, K, t, r, 0, flag)  # 个别期权求得的iv是无穷大，无效值，也无法求Greeks
    except:
        return np.nan
    if iv > 2:
        # print('IV 大于2')
...
def IV_GREEK(price, S, K, t, r, q, flag):
    ...
    q = 0
    try:
        iv = BSMiv.implied_volatility(price, S, K, t, r, q, flag)  # 个别期权求得的iv是无穷大，无效值，也无法求Greeks
        sigma = iv
...
def Instrument_Data(inst_id: str, tradingdate: str, time_interval: int = 0):
    ...
    if len(inst_id) > 6:  # 商品期权
        ...
        flag, K, underlying, expire_date = info[['options_type', 'strike_price', 'underlying_instr_id', 'expiredate']]
        ...
        q = 0
        inst_data.loc[:, 'ttm'] = inst_data.seq_no.apply(lambda x: ((total_ticks-x)/total_ticks + ttm)/243)

        inst_data_1 = inst_data.apply(lambda x: pd.Series(IV_GREEK(x['MidPrice'], x['Spot'], K, x['ttm'], r, q, flag))),
        ...
        try:
            ...
            inst_data = pd.read_csv(inst_path, usecols=['id', 'seq_no', 'bid_price1', 'ask_price1'],
                                    dtype={'id': object, 'seq_no': np.int64, 'bid_price1': np.float64,
                                           'ask_price1': np.float64},
                                    escapechar='/', na_values=r"\N")
        except:
        ...
        inst_data.rename(columns={'bid_price1': 'Bid1', 'ask_price1': 'Ask1'}, inplace=True)
